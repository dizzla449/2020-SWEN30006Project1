...
import automail.MailItem.MailType;
...
public class MailGenerator {
     ...
     */
    private MailItem generateRegularMail(){
        ...
        {
        	newMailItem = new MailItem(destinationFloor,arrivalTime,weight, MailType.REGULAR);      	
        } else {
        	newMailItem = new PriorityMailItem(destinationFloor,arrivalTime,weight,priorityLevel);
        }
        return newMailItem;
    }
    private MailItem generateFoodMail() {
    	MailItem newMailItem;
        int destinationFloor = generateDestinationFloor();
        int arrivalTime = generateArrivalTime();
        int weight = generateWeight();
    	newMailItem = new MailItem(destinationFloor,arrivalTime,weight, MailType.FOOD);      	
        return newMailItem;
    }
     ...
     */
    public void generateAllMail(Boolean FoodDeliveryEnabled){
        while(!complete){
        	createAndAddMailItem(MailType.REGULAR);
            if (!complete && FoodDeliveryEnabled) {
            	createAndAddMailItem(MailType.FOOD);
            }
        }
    }
    private void createAndAddMailItem(MailItem.MailType t) {
    	MailItem newMail = null;
    	switch(t) {
	    	case REGULAR:
	    		newMail =  generateRegularMail();
	    		break;
	    	case FOOD:
	    		newMail =  generateFoodMail();
	    		break;
    	}
